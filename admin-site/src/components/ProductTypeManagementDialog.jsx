...
import { getTaxes, createTax } from "@/services/api/taxService";
import {
	Select,
	SelectTrigger,
	SelectValue,
...
export function ProductTypeManagementDialog({ open, onOpenChange }) {
	...
	const [showArchivedProductTypes, setShowArchivedProductTypes] =
		useState(false);
		...
		try {
			const payload = {
				...
				default_markup_percent:
					...
					formData.default_markup_percent === null
						? null
						: Number(formData.default_markup_percent),
				standard_prep_minutes: Number(formData.standard_prep_minutes) || 0,
				max_quantity_per_item:
					formData.max_quantity_per_item === "" ||
					formData.max_quantity_per_item === null
						? null
				...
				<div className="flex-1 overflow-hidden">
					<div className="flex justify-between items-center mb-4">
						<Button
							...
							size="sm"
							onClick={() =>
								setShowArchivedProductTypes(!showArchivedProductTypes)
							}
						>
							{showArchivedProductTypes ? (
			...
			>
				<DialogContent className="sm:max-w-[95vw] max-h-[90vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Configure how this type of product behaves in your system - from
							inventory tracking to pricing and availability.
						</DialogDescription>
					</DialogHeader>
							...
							<div className="grid grid-cols-3 gap-6">
								<div className="space-y-3">
									...
									<p className="text-xs text-muted-foreground">
										A descriptive name for this product type
									</p>
								</div>
								<div className="space-y-3">
									...
									/>
									<p className="text-xs text-muted-foreground">
										Additional details about this product type
									</p>
								</div>
								<div className="space-y-3">
							...
							<div className="border rounded-lg p-4 space-y-4">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm text-muted-foreground">
											How stock is tracked and managed
										</p>
									</div>
								</div>
								...
								<div className="grid grid-cols-3 gap-6">
									<div className="space-y-3">
										...
										<Select
											value={formData.inventory_behavior}
											onValueChange={(v) =>
												setFormData({ ...formData, inventory_behavior: v })
											}
										>
											<SelectTrigger>
											...
											<SelectContent>
												<SelectItem value="NONE">
													No Tracking - Services, digital items
												</SelectItem>
												<SelectItem value="QUANTITY">
													Track Quantity - Retail products, beverages
												</SelectItem>
												<SelectItem value="RECIPE">
													Recipe Based - Food made from ingredients
												</SelectItem>
											</SelectContent>
										</Select>
										<p className="text-xs text-muted-foreground">
											How should inventory be calculated for this type of
											product?
										</p>
									</div>
									...
									<div className="space-y-3">
										...
										<Select
											value={formData.stock_enforcement}
											onValueChange={(v) =>
												setFormData({ ...formData, stock_enforcement: v })
											}
										>
											<SelectTrigger>
											...
											<SelectContent>
												<SelectItem value="IGNORE">
													Ignore - Always allow sales
												</SelectItem>
												<SelectItem value="WARN">
													Warn Only - Show warning but allow sale
												</SelectItem>
												<SelectItem value="BLOCK">
													Block - Prevent sales when out of stock
												</SelectItem>
											</SelectContent>
										</Select>
										<p className="text-xs text-muted-foreground">
							...
							<div className="border rounded-lg p-4 space-y-4">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Default tax and pricing behavior
										</p>
									</div>
								</div>
								...
								<div className="grid grid-cols-3 gap-6">
									<div className="space-y-3">
										...
										<div className="flex items-center gap-2">
											<Checkbox
												...
												onCheckedChange={(v) =>
													setFormData({ ...formData, tax_inclusive: !!v })
												}
											/>
											<Label
										...
										<p className="text-xs text-muted-foreground">
											Check if prices already include tax (common for B2C
											retail)
										</p>
									</div>
									...
									<div className="space-y-3">
										...
										<Select
											...
											onValueChange={(v) =>
												setFormData({ ...formData, pricing_method: v })
											}
										>
											<SelectTrigger>
											...
											<SelectContent>
												<SelectItem value="FIXED">
													Fixed Price - Manual price setting
												</SelectItem>
												<SelectItem value="COST_PLUS">
													Cost Plus - Automatic markup from cost
												</SelectItem>
											</SelectContent>
										</Select>
										<p className="text-xs text-muted-foreground">
											...
											<div className="relative">
												<Input
													...
													onChange={(e) =>
														setFormData({
															...formData,
															default_markup_percent: e.target.value,
														})
													}
													placeholder="25.00"
												/>
								...
								<div className="space-y-3">
																		...
																		>
																			...
																			<X
																				...
																				onClick={(e) => {
																					e.stopPropagation();
																					const next =
																						formData.default_taxes_ids.filter(
																							(id) => id !== taxId
																						);
																					setFormData({
																						...formData,
																						default_taxes_ids: next,
																					});
																				}}
																			/>
																		</Badge>
									...
									<p className="text-xs text-muted-foreground">
										These taxes will be applied to products of this type by
										default
									</p>
								</div>
							</div>
							...
							<div className="border rounded-lg p-4 space-y-4">
								<div className="flex items-center gap-3">
									...
									<div>
										...
										<p className="text-sm text-muted-foreground">
											Limits and restrictions for ordering
										</p>
									</div>
								</div>
								...
								<div className="grid grid-cols-3 gap-6">
									<div className="space-y-3">
										...
										<Input
											...
											onChange={(e) =>
												setFormData({
													...formData,
													max_quantity_per_item: e.target.value,
												})
											}
											placeholder="e.g., 5"
											min="1"
										/>
										<p className="text-xs text-muted-foreground">
											Maximum quantity of this item type per order (prevents
											bulk ordering issues)
										</p>
									</div>
									...
									<div className="space-y-3">
										...
										<div className="flex items-center gap-2">
											<Checkbox
												...
												onCheckedChange={(v) =>
													setFormData({
														...formData,
														exclude_from_discounts: !!v,
													})
												}
												id="exclude_from_discounts"
											/>
											<Label
												htmlFor="exclude_from_discounts"
												className="text-sm font-normal"
											>
												Block all discounts
											</Label>
										...
										<p className="text-xs text-muted-foreground">
											Check to prevent any discounts from applying to this
											product type (protects margins)
										</p>
									</div>
										...
										<div className="relative">
											<Input
												...
												onChange={(e) =>
													setFormData({
														...formData,
														standard_prep_minutes: e.target.value,
													})
												}
												placeholder="10"
												min="0"
			...
			>
				<DialogContent className="sm:max-w-md">
					<DialogHeader>
						...
						<DialogDescription>
							Create a new tax rate to use as a default for product types.
						</DialogDescription>
					</DialogHeader>
					<form
						onSubmit={async (e) => {
							...
							try {
								...
								if (Number.isNaN(payload.rate))
									throw new Error("Rate must be a number (e.g., 8.25)");
								...
								setFormData((prev) => ({
									...prev,
									default_taxes_ids: [
										...new Set([...(prev.default_taxes_ids || []), created.id]),
									],
								}));
								...
								toast({
									title: "Tax created",
									description: `${created.name} (${created.rate}%)`,
								});
							} catch (err) {
						...
						<div className="space-y-4">
							<div className="space-y-2">
								...
								<Input
									...
									value={taxForm.name}
									onChange={(e) =>
										setTaxForm((p) => ({ ...p, name: e.target.value }))
									}
									required
								/>
							</div>
							<div className="space-y-2">
								...
								<Input
									...
									value={taxForm.rate}
									onChange={(e) =>
										setTaxForm((p) => ({ ...p, rate: e.target.value }))
									}
									required
								/>
							</div>
