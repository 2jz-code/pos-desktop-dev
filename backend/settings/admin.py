...
class GlobalSettingsAdmin(admin.ModelAdmin):
    ...
    """
    fieldsets = (
        ("Store Information", {"fields": ("store_name", "store_address", "store_phone", "store_email")}),
        ("Financial Settings", {"fields": ("tax_rate", "surcharge_percentage", "currency")}),
        ...
        (
            "Web Order Notifications", {
                "fields": (
                    "web_order_notifications_enabled",
                    "web_order_auto_print_receipt",
                    "web_order_auto_print_kitchen",
                    "web_order_sound_notification",
                ),
                "classes": ("collapse",),
            }
        ),
        ("Business Hours", {"fields": ("opening_time", "closing_time", "timezone"), "classes": ("collapse",)}),
    )
    ...
    def changelist_view(self, request, extra_context=None):
        if self.model.objects.exists():
            ...
            return HttpResponseRedirect(reverse("admin:settings_globalsettings_change", args=[obj.pk]))
...
class PrinterConfigurationAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        return HttpResponseRedirect(reverse("admin:settings_printerconfiguration_change", args=[obj.pk]))
