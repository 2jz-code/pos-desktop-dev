...
def assign_existing_data_to_default_tenant(apps, schema_editor):
    ...
    1. Make all tenant FKs nullable ✅
    2. Run migrations ✅
    3. Assign existing data to default tenant ← YOU ARE HERE
    ...
    try:
        ...
        print(f"\n✓ Default tenant already exists: {default_tenant.name}")
    except Tenant.DoesNotExist:
        # Create default tenant if it doesn't exist (for fresh migrations)
        print(f"\n🆕 Creating default tenant: {default_tenant_slug}")
            ...
            if app_label == 'users' and model_name == 'User':
                ...
                for user in users_without_tenant:
                    try:
                        ...
                        skipped_count += 1
                        print(f"      ⚠ Skipped user {user.email} (username: {user.username}): {str(e)[:50]}")
...
def reverse_migration(apps, schema_editor):
    ...
    print(f"\n⚠️  REVERSING: Setting tenant to NULL for all records of: {default_tenant.name}")
