...
import { getAllActiveProducts } from "@/domains/products/services/productService";
		...
		try {
			...
			if (stockRecord) {
				...
				toast({
					title: "Error",
					description:
						"No stock record found for this product at the selected location.",
					variant: "destructive",
				});
			}
		} catch (error) {
		...
		try {
			...
			const payload = {
				...
				expiration_threshold: formData.expiration_threshold
					? parseInt(formData.expiration_threshold)
					: null,
			};
			...
			toast({
				title: "Error",
				description:
					error.response?.data?.message || "Failed to update stock record.",
				variant: "destructive",
			});
		} finally {
			setIsLoading(false);
				...
				>
					...
					<div className="space-y-2">
						...
						<Input
							...
							placeholder={
								currentStock
									? `Default: ${currentStock.effective_low_stock_threshold}`
									: "Default threshold"
							}
						/>
						<p className="text-xs text-slate-500">
							Alert when stock falls below this amount. Leave empty to use
							default{" "}
							{currentStock &&
								`(${currentStock.effective_low_stock_threshold})`}
						</p>
					</div>
					...
					<div className="space-y-2">
						...
						<Input
							...
							placeholder={
								currentStock
									? `Default: ${currentStock.effective_expiration_threshold} days`
									: "Default threshold"
							}
						/>
						<p className="text-xs text-slate-500">
							...
							{currentStock &&
								`(${currentStock.effective_expiration_threshold} days)`}
						</p>
					</div>
