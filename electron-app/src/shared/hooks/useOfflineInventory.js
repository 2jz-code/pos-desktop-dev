...
/**
 * Hook to fetch inventory stock data with offline cache support
 *
 * Strategy:
 * - Try cache first (instant), API is fallback only
 * - Provides refetch({ forceApi: true }) for post-mutation sync
 *
 * Note: This provides raw stock data. For filtered/paginated data when online,
 * the API should be used directly. This hook is primarily for offline display.
 *
		...
		async (fetchOptions = {}) => {
			...
			try {
				...
				if (forceApi && isOnline) {
					try {
						const { getAllStock } = await import(
						...
						console.error("[useOfflineInventory] Forced API fetch failed:", apiError);
						setError(apiError);
						return;
					}
				}
				...
				try {
					const cached = await window.offlineAPI.getCachedInventory();
					...
					console.warn("[useOfflineInventory] Cache read failed:", cacheError);
				}
				...
				if (isOnline) {
					try {
						...
						console.warn("[useOfflineInventory] API fallback failed:", apiError);
						setError(apiError);
					}
				}
			} finally {
