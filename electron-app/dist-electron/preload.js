...
contextBridge.exposeInMainWorld("offlineAPI", {
  ...
  getUserById: (userId) => ipcRenderer.invoke("offline:get-user-by-id", userId),
  getInventoryByProductId: (productId) => ipcRenderer.invoke("offline:get-inventory-by-product", productId),
  // Dataset versions
  getDatasetVersion: (datasetName) => ipcRenderer.invoke("offline:get-dataset-version", datasetName),
  getAllDatasetVersions: () => ipcRenderer.invoke("offline:get-all-dataset-versions"),
  // Queue operations
  queueOperation: (operation) => ipcRenderer.invoke("offline:queue-operation", operation),
  listPendingOperations: (filters) => ipcRenderer.invoke("offline:list-pending", filters),
  markOperationSynced: (operationId, serverResponse) => ipcRenderer.invoke(
    "offline:mark-synced",
    operationId,
    serverResponse
  ),
  markOperationFailed: (operationId, errorMessage) => ipcRenderer.invoke("offline:mark-failed", operationId, errorMessage),
  // Offline orders
  recordOfflineOrder: (orderPayload) => ipcRenderer.invoke("offline:record-order", orderPayload),
  getOfflineOrder: (localOrderId) => ipcRenderer.invoke("offline:get-order", localOrderId),
  listOfflineOrders: (status) => ipcRenderer.invoke("offline:list-orders", status),
  updateOfflineOrderStatus: (localOrderId, status, serverData) => ipcRenderer.invoke(
  ...
  // Offline approvals
  recordOfflineApproval: (approvalData) => ipcRenderer.invoke("offline:record-approval", approvalData),
  getUnsyncedApprovals: () => ipcRenderer.invoke("offline:get-unsynced-approvals"),
  // Metadata & stats
  ...
  checkLimitExceeded: (type, amount) => ipcRenderer.invoke("offline:check-limit", type, amount),
  ...
  storePairingInfo: (pairingInfo) => ipcRenderer.invoke("offline:store-pairing", pairingInfo),
