  ...
  for (const s of r.items) {
    ...
    if (T(e, p, `$${l.toFixed(2)}`), s.selected_modifiers_snapshot && s.selected_modifiers_snapshot.length > 0)
      for (const f of s.selected_modifiers_snapshot) {
        ...
        let v = `   - ${f.option_name}`;
        f.quantity > 1 && (v += ` (${f.quantity}x)`), parseFloat(f.price_at_sale) !== 0 ? T(e, v, `$${R.toFixed(2)}`) : e.println(v);
      }
  }
...
function ee(r, o = "KITCHEN", t = null) {
  ...
  n.alignCenter(), n.bold(!0), n.setTextSize(1, 1), n.println(`${o.toUpperCase()} TICKET`), n.setTextNormal(), n.bold(!1), n.alignLeft(), n.println(""), n.setTextSize(2, 2), n.bold(!0), n.println(`Order #${r.order_number || r.id}`), n.bold(!1), n.setTextNormal();
...
let m, C, x = null;
const O = P.env.VITE_DEV_SERVER_URL;
function ae() {
  ...
  }), O ? m.loadURL(O) : m.loadFile(_.join(P.env.DIST, "index.html")), m.on("closed", () => {
    m = null, C && C.close();
  });
}
function le() {
  ...
  }), O ? C.loadURL(`${O}customer.html`) : C.loadFile(_.join(P.env.DIST, "customer.html")), C.on("closed", () => {
    C = null;
  });
}
