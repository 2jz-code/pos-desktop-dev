...
async function formatReceipt(
	...
	const storeAddress =
		(storeSettings == null ? void 0 : storeSettings.store_address) ||
		"2105 Cliff Rd #300\nEagan, MN 55122";
	const storePhone =
		(storeSettings == null ? void 0 : storeSettings.store_phone) ||
		"(651) 412-5336";
	{
	...
	const customerName =
		...
		((_a = order.payment_details) == null ? void 0 : _a.customer_name) ||
		((_b = order.customer) == null ? void 0 : _b.full_name);
	if (customerName) {
	...
	for (const item of order.items) {
		...
		if (
			...
			for (const modifier of item.selected_modifiers_snapshot) {
				const modPrice =
					parseFloat(modifier.price_at_sale) *
					modifier.quantity *
					item.quantity;
	...
	if (!isTransaction) {
		let transactions =
			((_c = order.payment_details) == null ? void 0 : _c.transactions) || [];
	...
	printer.alignCenter();
	const receiptFooter =
		(storeSettings == null ? void 0 : storeSettings.receipt_footer) ||
...
function formatKitchenTicket(order, zoneName = "KITCHEN", filterConfig = null) {
	...
	const groupedItems = itemsToPrint.reduce((acc, item) => {
		...
		const categoryName = item.product
			? ((_a2 = item.product.category) == null ? void 0 : _a2.name) ||
			  "Miscellaneous"
			: "Custom Items";
		if (!acc[categoryName]) {
			acc[categoryName] = [];
	...
	for (const categoryName in groupedItems) {
		...
		for (const item of itemsInCategory) {
			...
			if (
				...
				const modifiersBySet = item.selected_modifiers_snapshot.reduce(
					(acc, modifier) => {
	...
	main = (function (e) {
		...
		function (e, r, t) {
			const { exec: n } = t(1),
				o = t(2).promisify(n);
			e.exports = {
				play: async (e2, r2 = 0.5) => {
					const t2 = "darwin" === process.platform ? Math.min(2, 2 * r2) : r2,
						n2 =
							"darwin" === process.platform
								? ((e3, r3) => `afplay "${e3}" -v ${r3}`)(e2, t2)
								: ((e3, r3) =>
										`powershell -c Add-Type -AssemblyName presentationCore; $player = New-Object system.windows.media.mediaplayer; ${((
											e4
										) => `$player.open('${e4}');`)(
											e3
										)} $player.Volume = ${r3}; $player.Play(); Start-Sleep 1; Start-Sleep -s $player.NaturalDuration.TimeSpan.TotalSeconds;Exit;`)(
										e2,
										t2
								  );
					try {
...
process$1.env.PUBLIC = app.isPackaged
	? process$1.env.DIST
	: path.join(process$1.env.DIST, "../public");
let mainWindow;
let customerWindow;
