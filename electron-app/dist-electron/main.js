import { app as w, ipcMain as _, screen as M, session as W, BrowserWindow as H } from "electron";
import b from "node:path";
import E from "node:process";
import { fileURLToPath as q } from "node:url";
import { createRequire as K } from "node:module";
import Y from "node-machine-id";
import A from "usb";
import Z from "child_process";
import G from "util";
const J = K(import.meta.url), Q = J("node-thermal-printer"), { printer: U, types: F } = Q, X = q(import.meta.url);
b.dirname(X);
function g(o, r, t) {
  o.leftRight(r, t);
}
async function ee(o, r = null, t = !1) {
  ...
  try {
    ...
    console.error("Could not print logo. Using text fallback."), console.error("Full logo printing error:", s), r != null && r.receipt_header && (e.println(r.receipt_header), e.println(""));
  }
  const n = (r == null ? void 0 : r.store_address) || `2105 Cliff Rd #300
Eagan, MN 55122`, i = (r == null ? void 0 : r.store_phone) || "(651) 412-5336";
  if (n.includes("\\n"))
    n.split("\\n").forEach((c) => {
  ...
  const a = o.order_number || o.id || "N/A", P = new Date(o.created_at).toLocaleString("en-US", {
    timeZone: "America/Chicago"
  }), p = o.customer_display_name || o.guest_first_name || ((y = o.payment_details) == null ? void 0 : y.customer_name) || ((f = o.customer) == null ? void 0 : f.full_name);
  p && e.println(`Customer: ${p}`), e.println(`Order #: ${a}`), e.println(`Date: ${P}`);
  const u = (o.dining_preference || "TAKE_OUT") === "DINE_IN" ? "Dine In" : "Take Out";
  ...
  t && (e.alignCenter(), e.bold(!0), e.println("--- TRANSACTION RECEIPT ---"), e.bold(!1), e.alignLeft(), o.status && e.println(`Order Status: ${o.status}`), e.println("** Payment Not Yet Processed **")), e.println(""), e.alignCenter(), e.bold(!0), e.println("ITEMS"), e.bold(!1), e.drawLine(), e.alignLeft();
  for (const s of o.items) {
    ...
    if (g(e, $, `$${c.toFixed(2)}`), s.selected_modifiers_snapshot && s.selected_modifiers_snapshot.length > 0)
      for (const m of s.selected_modifiers_snapshot) {
        const C = parseFloat(m.price_at_sale) * m.quantity * s.quantity;
        let S = `   - ${m.option_name}`;
        m.quantity > 1 && (S += ` (${m.quantity}x)`), parseFloat(m.price_at_sale) !== 0 ? g(e, S, `$${C.toFixed(2)}`) : e.println(S);
      }
  }
  if (e.drawLine(), g(e, "Subtotal:", `$${parseFloat(o.subtotal).toFixed(2)}`), parseFloat(o.total_discounts_amount) > 0 && g(
    e,
    "Discount:",
    `-$${parseFloat(o.total_discounts_amount).toFixed(2)}`
  ), parseFloat(o.total_surcharges || 0) > 0 && g(
    e,
    "Service Fee:",
    `$${parseFloat(o.total_surcharges).toFixed(2)}`
  ), g(e, "Tax:", `$${parseFloat(o.tax_total).toFixed(2)}`), parseFloat(o.total_tips || 0) > 0 && g(e, "Tip:", `$${parseFloat(o.total_tips).toFixed(2)}`), e.bold(!0), g(
    e,
    "TOTAL:",
    ...
    if (o.order_type === "WEB" && (s = s.filter((c) => c.status === "SUCCESSFUL")), s.length > 0) {
      e.bold(!0), e.println("Payment Details:"), e.bold(!1);
      for (const [c, d] of s.entries()) {
        const $ = (d.method || "N/A").toUpperCase(), m = parseFloat(d.amount || 0), C = parseFloat(d.surcharge || 0), S = parseFloat(d.tip || 0), N = (m + C + S).toFixed(2);
        if ($ === "CARD_ONLINE" || $ === "CARD_TERMINAL") {
          const D = d.card_brand || "", R = d.card_last4 || "";
          if (D && R) {
          ...
          g(e, ` ${$} (${c + 1})`, `$${N}`);
        if ($ === "CASH") {
          const D = parseFloat(d.cashTendered || 0).toFixed(2), R = parseFloat(d.change || 0).toFixed(2);
          parseFloat(D) > 0 && (g(e, "   Tendered:", `$${D}`), g(e, "   Change:", `$${R}`));
        }
      }
    }
  }
...
function ne(o, r = "KITCHEN", t = null) {
  ...
  n.println(""), n.println(""), n.println(""), n.println(""), n.alignCenter(), n.bold(!0), n.setTextSize(1, 1), n.println(`${r.toUpperCase()} TICKET`), n.setTextNormal(), n.bold(!1), n.alignLeft(), n.println(""), n.setTextSize(2, 2), n.bold(!0), n.println(`${o.order_number || o.id}`), n.bold(!1), n.setTextNormal();
  const i = o.customer_display_name || o.guest_first_name || ((u = o.payment_details) == null ? void 0 : u.customer_name) || ((v = o.customer) == null ? void 0 : v.full_name);
  ...
  n.println(`Time: ${a}`);
  const p = (o.dining_preference || "TAKE_OUT") === "DINE_IN" ? "DINE IN" : "TAKE OUT";
  ...
  for (const y in T) {
    n.bold(!0), n.underline(!0), n.println(`${y.toUpperCase()}:`), n.underline(!1), n.bold(!1);
    const f = T[y];
    for (const l of f) {
      n.bold(!0), n.setTextSize(1, 1);
      const s = l.product ? l.product.name : l.custom_name || "Custom Item";
      if (n.println(`${l.quantity}x ${s}`), n.setTextNormal(), n.bold(!1), l.selected_modifiers_snapshot && l.selected_modifiers_snapshot.length > 0) {
...
function re() {
  return B || (B = 1, L = function(o) {
    ...
    return t.m = o, t.c = r, t.d = function(e, n, i) {
      ...
      if (t.r(i), Object.defineProperty(i, "default", { enumerable: !0, value: e }), 2 & n && typeof e != "string") for (var a in e) t.d(i, a, (function(P) {
        return e[P];
      }).bind(null, a));
      return i;
    }, t.n = function(e) {
    ...
    o.exports = { play: async (i, a = 0.5) => {
      const P = process.platform === "darwin" ? Math.min(2, 2 * a) : a, p = process.platform === "darwin" ? ((T, u) => `afplay "${T}" -v ${u}`)(i, P) : ((T, u) => `powershell -c Add-Type -AssemblyName presentationCore; $player = New-Object system.windows.media.mediaplayer; ${((v) => `$player.open('${v}');`)(T)} $player.Volume = ${u}; $player.Play(); Start-Sleep 1; Start-Sleep -s $player.NaturalDuration.TimeSpan.TotalSeconds;Exit;`)(i, P);
      try {
        await n(p);
      } catch (T) {
        throw T;
...
var ie = re();
const se = /* @__PURE__ */ oe(ie), { machineIdSync: ae } = Y, k = K(import.meta.url), ce = q(import.meta.url), j = b.dirname(ce), le = E.env.NODE_ENV === "development";
...
function de() {
  ...
  h = new H({
    ...
    h == null || h.webContents.send(
      "main-process-message",
      (/* @__PURE__ */ new Date()).toLocaleString()
    );
  }), O ? h.loadURL(O) : h.loadFile(b.join(E.env.DIST, "index.html")), h.on("closed", () => {
    h = null, I && I.close();
  });
}
function pe() {
  const r = M.getAllDisplays().find(
  ...
  }), O ? I.loadURL(`${O}customer.html`) : I.loadFile(b.join(E.env.DIST, "customer.html")), I.on("closed", () => {
    I = null;
  });
}
...
_.handle("play-notification-sound", async (o, r) => {
  try {
    const t = r || "notification.wav", e = b.join(E.env.PUBLIC, "sounds", t);
    return console.log(`[IPC] Attempting to play sound: ${e}`), await se.play(e), { success: !0 };
  } catch (t) {
...
async function V(o, r) {
  let t = null;
  try {
    const e = parseInt(o.vendorId || o.vendor_id, 10), n = parseInt(o.productId || o.product_id, 10);
    if (!e || !n)
      throw new Error(
...
_.handle(
  "print-receipt",
  async (o, { printer: r, data: t, storeSettings: e, isTransaction: n = !1 }) => {
    console.log(`
--- [Main Process] Using HYBRID print method ---`), console.log(
    ...
    try {
      ...
      ), await V(r, i), console.log("[Main Process] Hybrid print command sent successfully."), { success: !0 };
    } catch (i) {
...
--- [Main Process] KITCHEN TICKET HANDLER for zone: "${e}" ---`
    ...
    try {
      ...
      const i = k("node-thermal-printer"), { printer: a, types: P } = i;
      let p = new a({
        type: P.EPSON,
...
--- [Main Process] TESTING NETWORK PRINTER at: ${r} ---`
  ...
  try {
    ...
    if (await i.isPrinterConnected())
      return console.log(`SUCCESS: Connection to ${r} is OK.`), i.println("Connection Test OK"), i.cut(), await i.execute(), {
        success: !0,
...
--- [Main Process] Using HYBRID open-drawer method ---`);
  try {
    const e = A.getDeviceList().find(
      (a) => (a.product || `USB Device ${a.deviceDescriptor.idVendor}:${a.deviceDescriptor.idProduct}`) === r
    );
    if (!e)
    ...
    ), await V(n, i), console.log("[Main Process] Hybrid open-drawer command sent successfully."), { success: !0 };
  } catch (t) {
...
_.handle("get-session-cookies", async (o, r) => {
  try {
    const { session: t } = k("electron"), e = await t.defaultSession.cookies.get({ url: r });
    console.log(`[Main Process] Found ${e.length} cookies for ${r}`), e.forEach((i, a) => {
      console.log(
...
_.handle("get-machine-id", () => ae({ original: !0 }));
_.on("shutdown-app", () => {
  w.quit();
});
w.whenReady().then(async () => {
  console.log("[Main Process] Starting Electron app - online-only mode"), console.log("[Main Process] Hardware acceleration and display settings applied at startup"), de(), pe();
});
